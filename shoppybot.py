...
(BOT_STATE_INIT,
 ...
 ADMIN_TXT_DELETE_COURIER) = range(19)
...
def create_service_notice_keyboard(update, user_id, order_id, user_data):
    buttons = []
    ...
    #     buttons.append([
    #         InlineKeyboardButton(_('Assign @{}').format(courier_nickname),
    #                              callback_data='courier|{}|{}'.format(user_id, courier_nickname))
    #     ])

    if user_data['shipping'].get('pickup_location'):
        location_name = user_data['shipping']['pickup_location']
        couriers = db.get_couriers_for_location_name(location_name)
        courier_nickname = update.message.from_user.username

        # if couriers.

    buttons.append([
        InlineKeyboardButton(_('Take Responsibility'),
                             callback_data='courier|{}|{}|{}'.format(user_id, order_id))
    ])
    return InlineKeyboardMarkup(buttons)

def create_drop_responsibility_keyboard(user_id, courier_nickname):
    buttons = [
        InlineKeyboardButton(_('Assigned to @{}'.format(courier_nickname)),
                             url='https://t.me/{}'.format(courier_nickname)),
        InlineKeyboardButton(_('Drop responsibility'), callback_data='dropped'),
    ]
    return InlineKeyboardMarkup([buttons])

def resend_responsobility_keyboard(bot, update, user_data):
...
def on_confirm_order(bot, update, user_data):
    ...
    if key == BUTTON_TEXT_CONFIRM:
        ...
        bot.send_message(
            ...
            parse_mode=ParseMode.HTML,
            reply_markup=create_service_notice_keyboard(update, user_id, order_id, user_data),
        )
        bot.send_message(
...
def service_channel_courier_query_handler(bot, update, user_data):
    ...
    bot.send_message(
        config.get_service_channel(),
        text=query.message.text, parse_mode=ParseMode.HTML,
        reply_markup=create_drop_responsibility_keyboard(user_id, courier_nickname),
    )
...
def main():
    ...
    updater.dispatcher.add_handler(
        CallbackQueryHandler(resend_responsobility_keyboard,
                             pattern='^dropped', pass_user_data=True))
